...
按需导入句法对子包无效。如果导入了 `java.util` 包，仍然必须使用完全限定名称`java.util.zip.ZipInputStream` 引用这个类。按需导入类型和一个一个导入包中的所有类型作用不一样。按需导入更像是使用单个类型导入句法把代码中真正用到的各种类型从包中导入命名空间，因此才叫“按需”导入——
用到某个类型时才会将其导入。